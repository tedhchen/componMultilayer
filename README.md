...
2. \*Install the required packages (see (https://github.com/tedhchen/componMultilayer/blob/main/compontwitter.yml)[`compontwitter.yml`]).
3. \*Change (https://github.com/tedhchen/componMultilayer/blob/main/scripts/config.ini.template)[`config.ini.template`] into `config.ini` and enter your own settings.
...
## Protocol
Details about data collection and labeling for each of the steps outline below can be found in the (https://github.com/tedhchen/componMultilayer/blob/main/TwitterCodebook.pdf)[`TwitterCodebook.pdf`].
...
In this step, you should find the collective main account of each of your policy actors and provide a list of keyword for filtering the side accounts. Document them in the template file ((https://github.com/tedhchen/componMultilayer/blob/main/templates/main_accounts.csv)[`template\main_accounts.csv`]) and assign them with `level` 0. Make sure this file is encoded in UTF-8.

### 02. Identify individual main
In this step, you should find the individual main accounts of each of your policy actors. Document them in the template file ((https://github.com/tedhchen/componMultilayer/blob/main/templates/main_accounts.csv)[`template\main_accounts.csv`]) and assign them with `level` 1. Make sure this file is encoded in UTF-8.

### 0a. Standardize formatting of main accounts
This step needs to be done before step 05. Doing it as early as possible after steps 01 and 02 will ensure the most consistent data (so accounts do not change their handles). Run the Jupyter Notebook (https://github.com/tedhchen/componMultilayer/blob/main/scripts/0a_standardize_mains.ipynb)[`0a_standardize_main.ipynb`]. The output file will be used by a later step, but can be ignored for now.

### 03. Identify side accounts
Only do this step after `main_accounts.csv` has been filled with the collective main accounts and their keywords (step 01). Run the Jupyter Notebook (https://github.com/tedhchen/componMultilayer/blob/main/scripts/03_identify_sides.ipynb)[`03_identify_sides.ipynb`]. It will create coding sheets for each of your collective main accounts (as long as they have potential side accounts).
...
### 05. Merge all levels
After all preceding steps are complete, run the Jupyter Notebook (https://github.com/tedhchen/componMultilayer/blob/main/scripts/05_levels_merge.ipynb)[`05_levels_merge.ipynb`]. This will create an output file `all_accounts.csv`, which contains all policy actors' accounts and their levels. We will use this to collect Twitter behavior of our policy actors.
